...
- Diagrams are fenced ` ```mermaid ` blocks, only where they show a mechanism the text cannot, and never more than one per page. Offline, the browser shows the block's source in place of the diagram, so its labels read as plain text.
